...
## 3. Миграция данных 
Расширьте JSON-схему, теперь помимо описания полей таблиц в ней будут находится данные в поле `data`, например:
```
[
  { 
     "name": "player",
...
При запуске программы вы должны обновить данные таблиц:
- если в БД нет записи с указанным в JSON-файле `id`, запись нужно создать
- если в БД уже есть запись с указанным `id`, то ее нужно обновить
- записи, которых нет в JSON-файле нужно удалить

Удаление записей должно происходить только с теми, записями, которых нет в JSON-файле

Например, до миграции в таблице `player_in_game` были следующие данные:
...
После миграции данных вы должны получить:

Id | player_id | game_id | is_winner | Действие
--|-----------|-----------|---------- | -----
